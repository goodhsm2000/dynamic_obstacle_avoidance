...
#include <costmap_2d/VirtualLayerConfig.h>
...
// 사람의 예상 경로가 담겨있는 토픽
#include <object_tracking_msgs/Trajectory.h>
...
public:
    ...
    // Polygon이란 이름으로 std::vector<geometry_msgs::Point> 타입을 지칭하도록 정의
...
private:
    ...
    void reconfigureCb(costmap_2d::VirtualLayerConfig &config, uint32_t level);
    ...
    // 사람의 경로를 costmap에 올리기위한 함수
    void pathCallback(const object_tracking_msgs::TrajectoryConstPtr &path_msg);
    ...
    std::shared_ptr<dynamic_reconfigure::Server<costmap_2d::VirtualLayerConfig>> _dsrv; // dynamic_reconfigure server for the costmap
    ...
    std::vector<int> target_obj;
    std::vector<int> cur_obj;
    bool init = true;
};
