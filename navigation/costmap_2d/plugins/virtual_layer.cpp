...
void VirtualLayer::onInitialize()
{
    ...
    // 검출한 객체의 예상 경로를 layer에 올리기 위한 작업
    param = "path_topics";
    parseTopicsFromYaml(nh, param);
...
{   
    // XmlRpcValue를 사용하여 YAML 형식의 매개변수를 저장할 변수를 선언
    XmlRpc::XmlRpcValue param_yaml;

    // ROS 노드 핸들을 통해 매개변수 서버에서 지정된 매개변수를 읽어옴 
    // 읽어온 매개변수를 param_yaml 변수에 저장하고, 매개변수를 성공적으로 읽어왔는지 확인
    if (nh.getParam(param, param_yaml)) {

        // 읽어온 매개변수가 유효한지 확인, 유효하지 않거나 배열이 아닌 경우 오류 메시지를 출력하고, 예외를 발생
        ...
        for (std::size_t i = 0; i < param_yaml.size(); ++i) {
            ...
            if (param_yaml[i].getType() != XmlRpc::XmlRpcValue::TypeString) {
                ...
                // 현재 요소를 문자열로 바꾸고 topic 형식이 잘못되었다면 수정
                std::string topic_name(param_yaml[i]);
                if ((topic_name.empty()) && (topic_name.at(0) != '/')) {
                    topic_name = "/" + topic_name;
                }

                // param이 zone_topics나 obstacle_topics면 해당 topic을 subscribe
                ...
                // custom 추가
                else if (param.compare("path_topics") == 0) {
                    _subs.push_back(nh.subscribe(topic_name, 100, &VirtualLayer::pathCallback, this));
                }
...
// YAML 형식의 매개변수를 읽어와서, 그에 따라 다각형(polygons), 선(lines), 점(points)들을 파싱하고 필요한 데이터 구조에 저장하는 함수
                ...
                if (param_yaml[i].getType() == XmlRpc::XmlRpcValue::TypeArray) {
                    if (param_yaml[i].size() == 1) { // add a point
                        try {
                            ...
                            // param_yaml 값을 geometry_msgs::Point로 변환
    ...
    try {
        if (val.getType() == XmlRpc::XmlRpcValue::TypeArray && val.size() == 2) {
            auto convDouble = [](const XmlRpc::XmlRpcValue &val) -> double {
                auto val_copy = val;
                if (val_copy.getType() == XmlRpc::XmlRpcValue::TypeInt) 
                {
...
void VirtualLayer::polygonOutlineCells(const std::vector<PointInt> &polygon, std::vector<PointInt> &polygon_cells)
{
    ...
    // polygon 영역을 모두 costmap에 올릴 것이 아니기 때문에 필요 x 
    // if (!polygon.empty()) {
...
void VirtualLayer::rasterizePolygon(const std::vector<PointInt> &polygon, std::vector<PointInt> &polygon_cells, bool fill)
{
    ...
    // polygon 영역 전체를 costmap에 추가할 경우 사용
    // PointInt swap;
    // unsigned int i = 0;
...
void VirtualLayer::obstaclesCallback(const custom_msgs::ObstaclesConstPtr &obstacles_msg)
{
    ...
    ROS_INFO_STREAM(obstacles_msg->list[0].form[0] << "message type");
    ROS_INFO_STREAM(obstacles_msg->list[0].form.size() << "form size");   
...
void VirtualLayer::pathCallback(const object_tracking_msgs::TrajectoryConstPtr &path_msg)
{
    if (_clear_obstacles) {
        _obstacle_polygons.clear();
    }

    // if ((target_obj == cur_obj) || init) {
    ...
    //     for(int i = 0; i < path_msg->cur_tracked_id.size(); i++) {
    //         int id;
    //         id = path_msg->cur_tracked_id[i];
    //         target_obj.push_back(id);
    //         sort(target_obj.begin(), target_obj.end());
    //     }

    //     init = false;
    // }

    ROS_INFO("path_size: %d", path_msg -> paths.size());

    for (int i = 0; i < path_msg -> paths.size(); i++) {
        Polygon vector_to_add;
        for (int j = 0; j < path_msg->paths[i].path_list.size(); j++) {
            geometry_msgs::Point pt;
            pt = path_msg -> paths[i].path_list[j];
            vector_to_add.push_back(pt);
        }
        _obstacle_polygons.push_back(vector_to_add);
    }
    ...
    // cur_obj.push_back(path_msg->object_id);
    // sort(cur_obj.begin(), cur_obj.end());

    // _obstacle_polygons.push_back(vector_to_add);

    // ROS_INFO("target_obj: ");
    // for ( int i = 0; i < target_obj.size(); i++) {
    //     ROS_INFO("%d", target_obj[i]);
    // }

    // ROS_INFO("cur_obj: ");
    // for ( int i = 0; i < cur_obj.size(); i++) {
    //     ROS_INFO("%d", cur_obj[i]);
    // }  
    // ROS_INFO_STREAM(obstacles_msg->list[0].form[0] << "message type");
    // ROS_INFO_STREAM(obstacles_msg->list[0].form.size() << "form size");   
    // for (int i = 0; i < obstacles_msg->list.size(); ++i) {
