...
class VelKalman:
    ...
    def calvel(self, yaw):
        ...
        wvelx = velx * math.cos(yaw) + vely * math.cos(math.pi / 2 + yaw)
        wvely = velx * math.sin(yaw) + vely * math.sin(math.pi / 2 + yaw)

        vel = math.sqrt((wvelx ** 2) + (wvely ** 2))

        return vel, wvelx, wvely
    ...
    def predict_path(self, t, world_x, world_y, yaw):
        ...
        vel, wvelx, wvely = self.calvel(yaw)
        ...
        for step in range(t):
            ps = Point()
            ps.x = x + wvelx * dt * step
            ps.y = y + wvely * dt * step
            path_list.append(ps)
